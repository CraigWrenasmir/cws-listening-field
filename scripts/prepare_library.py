...
for p in cat:
 ...
 if p['op']>=asset_policy['ranges'][0]['first_opus']:
  asset_range=next((r for r in asset_policy['ranges'] if r['first_opus']<=p['op']<=r['last_opus']),None)
  assert asset_range,'Add a new immutable asset range before extending the catalogue; preserve existing URLs'
