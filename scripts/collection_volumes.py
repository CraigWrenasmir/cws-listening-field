...
def build_volumes(root, catalog, selected=None):
    ...
    for number,pieces in sorted(groups.items()):
        ...
        rules=series_for(ops[0]);assert all(series_for(op)['id']==rules['id'] for op in ops)
        ...
        if not unchanged:
            ...
            writer.add_metadata({'/Title':f'CWS {rules["label"]} - Volume {number} - Op. {ops[0]}–{ops[-1]}','/Author':'CWS Library - studies with Maple (AI)'})
            ...
            with zipfile.ZipFile(zip_path,'w',zipfile.ZIP_DEFLATED) as archive:
                ...
                for name in ['data/STYLE.md','data/EXPERIMENTAL_ARC.md','data/series.json','licenses/Leipzig-OFL.txt','licenses/GeneralUser-GS.txt']:
                    archive.write(root/name,name)
                archive.write(pdf_path,pdf_name)
                listing='\n'.join(f'CWS Op. {p["op"]} — {p["title"]}' for p in pieces)
                archive.writestr('VOLUME.txt',f'{rules["header"]}\nVolume {number:02d}\nOp. {ops[0]}–{ops[-1]}\n\n{listing}\n\nScores, recordings, MIDI, MusicXML and source metadata.\n')
            with zipfile.ZipFile(zip_path) as archive:assert archive.testzip() is None
        pages=sum(p['pages'] for p in pieces)
        volume=dict(number=number,series=rules['id'],series_label=rules['label'],first=ops[0],last=ops[-1],ops=ops,count=len(ops),pages=pages,
                    first_title=pieces[0]['title'],last_title=pieces[-1]['title'],
                    complete=len(ops)==VOLUME_SIZE or ops[-1]==rules['last_opus'],seconds=round(sum(p['duration_seconds'] for p in pieces),2),
...
<meta name="color-scheme" content="light dark"><title>Collected volumes · The Listening Field</title>
<link rel="stylesheet" href="../src/downloads.css"></head><body><main>
<header><span>CWS / STUDIES</span><a href="../index.html">Return to the field</a></header>
