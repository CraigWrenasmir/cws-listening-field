...
import {localAssetPath,assetPrefix} from './asset-paths.mjs';
...
for(const p of catalog)for(const f of [p.pdf,p.audio,p.midi,p.xml,...p.scores])assert((await readFile(path.join(f.startsWith(assetPrefix)?root:out,localAssetPath(f)))).length>0,'Missing runtime asset: '+f);
