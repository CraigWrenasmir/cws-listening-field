...
for(const portrait of [false,true]){
 ...
 click('kf-zoom-in');click('kf-zoom-in');click('kf-zoom-in');assert(Number(svg.dataset.zoom)>2);assert(svg.classList.contains('is-close'));assert.equal(all().length,data.length);assert.equal(edges().length,layout.links.length,'Zoom must never filter relationships');
 ...
 click('kf-fit');assert.equal(svg.dataset.zoom,'1.000');assert(!svg.classList.contains('is-close'));assert.equal(all().length,data.length);
