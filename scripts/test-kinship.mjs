...
for(const portrait of [false,true]){
 ...
 click('kf-save');assert.equal(document.getElementById('kf-save').getAttribute('aria-pressed'),'true');assert(touched.classList.contains('is-saved'));assert(document.querySelector('#kf-favourites [data-saved-op="3"]'));assert.equal(all().length,data.length);assert.equal(edges().length,layout.links.length,'Saving must preserve the complete map');click('kf-save');assert(!touched.classList.contains('is-saved'));assert.equal(document.querySelectorAll('#kf-favourites li').length,0);
