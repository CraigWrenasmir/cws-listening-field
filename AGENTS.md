...
- Explicit bar definitions are in `scripts/compose.py` and `scripts/new_pieces.py`. Select opus numbers when running the music pipeline so an addition does not regenerate approved earlier pieces.
