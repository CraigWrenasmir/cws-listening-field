...
- Optional `rh_inner` rows create a separately notated right-hand inner voice. Use explicit `motif.voice` and `ancestry.source_voice` when selecting fragments from a polyphonic hand. Validate simultaneous whole-hand reach and pitch overlap, not just individual chord shapes. Preserve each voice in notation, playback, highlights and the sculpture.
- Collection downloads use `scripts/collection_volumes.py`, with up to 24 works per PDF/ZIP volume. Keep the download index complete and earlier closed volumes stable. `npm run check` validates coverage, file sizes and the Pages footprint. Respect the documented size thresholds before pushing.
